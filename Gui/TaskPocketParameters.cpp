...
TaskPocketParameters::TaskPocketParameters(ViewProviderPocket *PocketView,QWidget *parent, bool newObj)
    : TaskSketchBasedParameters(PocketView, parent, "PartDesign_Pocket",tr("Cut parameters"))
    , oldLength(0)
{
    ...
    // Part
    for (const auto& t : pcPocket->Part.getEnumVector()) 
        ui->keptPart->addItem(QString::fromUtf8(t.c_str()));

	ui->keptPart->setCurrentIndex(pcPocket->Part.getValue());
    // Set object labels
    ...
    connect(ui->keptPart, SIGNAL(currentIndexChanged(int)),
            this, SLOT(onKeptPartChanged(int)));
...
void TaskPocketParameters::onKeptPartChanged(int index)
{
    PartDesign::Pocket* pcPocket = static_cast<PartDesign::Pocket*>(vp->getObject());
    pcPocket->Part.setValue(long(index));
    recomputeFeature();
}
