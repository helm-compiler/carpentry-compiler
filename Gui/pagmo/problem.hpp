...
struct null_problem {
     ...
     */
    vector_double fitness(const OptCompiler::EncodedVec &) const
    {
...
{
    template <typename U>
    using fitness_t = decltype(std::declval<const U &>().fitness(std::declval<const OptCompiler::EncodedVec &>()));
...
struct prob_inner_base {
    ...
    virtual vector_double fitness(const OptCompiler::EncodedVec &) const = 0;
...
struct prob_inner final : prob_inner_base {
    ...
    // Mandatory methods.
    virtual vector_double fitness(const OptCompiler::EncodedVec &dv) const override final
    {
...
public:
     ...
     */
    vector_double fitness(const OptCompiler::EncodedVec &dv) const
    {
     ...
     */
    bool feasibility_x(const OptCompiler::EncodedVec &x) const
    {
