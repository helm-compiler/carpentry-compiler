...
public:
    ...
    typedef std::vector<OptCompiler::EncodedVec>::size_type size_type;
    /// Default constructor
    /**
...
//         }
	...
	void randomize_tree_node(std::shared_ptr<OptCompiler::TreeNode> node)
	{
		if (!node) return;

		if (node->GetN() != -1)
		{
			node->SetN(uniform_integral_from_range_cm(0, node->ec->ens.size() - 1, m_e));
		}

		for (auto& n : node->subNodes)
		{
			randomize_tree_node(n);
		}
	}

    int nbbb = 0;

	bool random_decision_tree(OptCompiler::EGraph* eg, OptCompiler::ENode* en, size_t pop_size, const std::vector<std::vector<std::tuple<string, int, int>>>& encoded_vecs, const VectorPI2& encoded_tds)
	{
        int secondPass = pop_size;
		for (int i = 0; i < encoded_vecs.size() && size() < secondPass; i++)
		{
			OptCompiler::EncodedVec vec(eg, en, encoded_vecs[i], encoded_tds[i]);
			push_back(vec);
		}

		while (size() < secondPass || size() % 4u != 0)
		{
			OptCompiler::EncodedVec vec(en);
			randomize_tree_node(vec.root);
			push_back(vec);
            nbbb++;
		}
...
public:
     ...
     */
    void push_back(const OptCompiler::EncodedVec &x)
    {
     ...
     */
    void push_back(OptCompiler::EncodedVec &&x)
    {
     ...
     */
    void push_back(const OptCompiler::EncodedVec &x, const vector_double &f)
    {
        push_back_impl(x, f);
    }
     ...
     * @throws unspecified any exception thrown by the previous ``push_back()`` overload.
     */
    void push_back(OptCompiler::EncodedVec &&x, vector_double &&f)
    {
	 ...
	 */
	void set_xf(size_type i, const OptCompiler::EncodedVec& x, const vector_double& f)
    {
        if (i >= size()) {
        ...
        // can throw.
        m_f[i].resize(f.size());
		m_x[i] = x; //TODO: STD::MOVE
        std::copy(f.begin(), f.end(), m_f[i].begin());
	}



	OptCompiler::EncodedVec champion_x() const
	{
		return m_champion_x;
	}
     ...
     */
    void set_x(size_type i, const OptCompiler::EncodedVec &x)
    {
     ...
     */
    const std::vector<OptCompiler::EncodedVec> &get_x() const
    {
        return m_x;
    }
	...
	void ExportPareto(std::ostream& os, const population& p)
	{
		for (size_type i = 0u; i < p.size(); ++i) {
			stream(os, "", p.m_f[i], '\n');
		}
...
private:
    ...
    void update_champion(OptCompiler::EncodedVec x, vector_double f)
    {
        assert(f.size() > 0u);
    ...
    // Decision vectors.
    std::vector<OptCompiler::EncodedVec> m_x;
    // Fitness vectors.
    std::vector<vector_double> m_f;
    // The Champion chromosome
	OptCompiler::EncodedVec m_champion_x;
