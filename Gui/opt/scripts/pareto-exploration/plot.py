...
def mk_fc_fp_plot(data, edata, a):
    ...
    a.set_xlim(min(min(fc),min(efc)) - 0.1 * xLength, max(max(fc),max(efc)) + 0.1 * xLength)
    a.set_ylim(min(min(fp),min(efp)) - 0.1 * yLength, max(max(fp),max(efp)) + 0.1 * yLength)

def mk_fc_ft_plot(data, edata, a):
    ...
    a.set_xlim(min(min(fc),min(efc)) - 0.1 * xLength, max(max(fc),max(efc)) + 0.1 * xLength)
    a.set_ylim(min(min(ft), min(eft)) - 0.1 * yLength, max(max(ft), max(eft)) + 0.1 * yLength)

def mk_ft_fp_plot(data, edata, a):
    ...
    yLength = max(ft) - min(ft)
    a.set_xlim(min(min(fp), min(efp)) - 0.1 * xLength, max(max(fp), max(efp)) + 0.1 * xLength)
    a.set_ylim(min(min(ft), min(eft)) - 0.1 * yLength, max(max(ft), max(eft))+ 0.1 * yLength)

def mk_fc_fp_ft_plot(data, edata, ax):
    fc = list(map(lambda t: t[0], data))
    fp = list(map(lambda t: t[1], data))
...
def main():
    ...
    mk_fc_fp_ft_plot(fc_fp_ft, cfc_fp_ft, plt.subplot2grid((30, 40), (0, 0), rowspan=30, colspan=22, projection='3d'))
    mk_fc_fp_plot(fc_fp, cfc_fp, plt.subplot2grid((30, 40), (0, 25), rowspan=7, colspan=12))
    mk_fc_ft_plot(fc_ft, cfc_ft, plt.subplot2grid((30, 40), (10, 25), rowspan=7, colspan=12))
    mk_ft_fp_plot(fp_ft, cfp_ft,plt.subplot2grid((30, 40), (20, 25),  rowspan=7,colspan=12))
