  ...
  : TaskBox(QPixmap(),tr("Primitive parameters"), true, parent), vp(vp)
{
    ...
    switch(static_cast<PartDesign::FeaturePrimitive*>(vp->getObject())->getPrimitiveType()) {
        ...
        case PartDesign::FeaturePrimitive::Box:
            ...
            for (const auto& t : static_cast<PartDesign::Box*>(vp->getObject())->Type.getEnumVector()) ui.boxType->addItem(QString::fromUtf8(t.c_str()));
	    ui.boxType->setCurrentIndex(static_cast<PartDesign::Box*>(vp->getObject())->Type.getValue());
    ...
    connect(ui.boxType, SIGNAL(currentIndexChanged(int)), this, SLOT(onBoxTypeChanged(int)));
...
void TaskBoxPrimitives::onBoxTypeChanged(int index)
{
    PartDesign::Box* box = static_cast<PartDesign::Box*>(vp->getObject());
    box->Type.setValue(long(index));
